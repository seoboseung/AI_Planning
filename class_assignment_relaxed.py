#!/usr/bin/env python3
"""
class_assignment_relaxed.py - 성적 균형 완화 버전

성적을 완벽하게 균형 맞추지 않고 적당한 수준으로 완화한 버전
- 성적 완벽 균형 → 성적 적당한 균형 (±5% 정도 허용)
- 다른 제약조건들은 그대로 유지

Usage:
    python class_assignment_relaxed.py --input students.csv --output assignment.csv --mode ortools
"""
...
def build_previous_classmates(df, n):
    """전년도 클래스메이트 관계를 구축"""
    previous_pairs = []
    ...
    for idx, row in df.iterrows():
        prev_class = str(row.get('24년 학급', '')).strip().lower()
    ...
    # 같은 클래스였던 학생들 간의 관계 생성
    total_relationships = 0
    for class_name, students in class_groups.items():
        if len(students) > 1:
            for i in range(len(students)):
                for j in range(i+1, len(students)):
                    previous_pairs.append((students[i], students[j]))
                    total_relationships += 1
    
    print(f"전년도 클래스 분석:")
    for class_name, students in class_groups.items():
        print(f"  {class_name}반: {len(students)}명")
    print(f"  총 분리해야 할 관계: {total_relationships}건")
    
    return previous_pairs

def run_ortools_relaxed(df, out_path, class_sizes):
    try:
        ...
        if isinstance(enemies_list, list):
            for j in enemies_list:
                if j>=0 and j<n and j!=i:
                    ...
                    enemy_constraints += 1
    print(f"Added {enemy_constraints} enemy separation constraints (HARD)")

    # 제약조건 4: 전년도 클래스메이트 분리 (하드 제약)
    previous_pairs = build_previous_classmates(df, n)
    previous_constraints = 0
    for i, j in previous_pairs:
        for c in range(k):
            model.Add(x[(i,c)] + x[(j,c)] <= 1)
        previous_constraints += 1
    print(f"Added {previous_constraints} previous classmate separation constraints (HARD)")

    # 제약조건 5: 리더십 분배 - 각 학급에 최소 1명씩 (하드 제약)
    leader_idxs = [i for i in range(n) if df.at[i,'is_leader']==1]
    if leader_idxs:
        for c in range(k):
            model.Add(sum(x[(i,c)] for i in leader_idxs) >= 1)
        print(f"Added leadership constraints: {len(leader_idxs)} leaders, min 1 per class (HARD)")

    # 제약조건 6: 피아노 학생 균등 분배 (하드 제약)
    ...
    if piano_idxs:
        ...
        print(f"Added piano balance constraints: {p_total} piano students, {p_floor}-{p_ceil} per class (HARD)")

    # 제약조건 7: 비등교자 균등 분배 (하드 제약)
    ...
    if atrisk_idxs:
        ...
        print(f"Added at-risk balance constraints: {a_total} at-risk students, {a_floor}-{a_ceil} per class (HARD)")

    # 제약조건 8: 남녀 비율 균등 분배 (하드 제약)
    ...
    if male_idxs:
        ...
        print(f"Added gender balance constraints: {m_total} male students, {m_floor}-{m_ceil} per class (HARD)")

    # 제약조건 9: 운동 능력 균등 분배 (하드 제약)
    ...
    if athletic_idxs:
        ...
        print(f"Added athletic balance constraints: {ath_total} athletic students, {ath_floor}-{ath_ceil} per class (HARD)")

    # 제약조건 10: 부활동/클럽 활동 균등 분배 (하드 제약)
    ...
    if club_idxs:
        ...
        print(f"Added club balance constraints: {club_total} club students, {club_floor}-{club_ceil} per class (HARD)")

    # 제약조건 11: 성적 균형 분배 (완화된 소프트 제약) - 임시 주석처리
    # # 성적을 정수로 변환 (OR-Tools는 정수만 허용)
    # grades_int = [int(round(grade)) for grade in df['grade_val'].tolist()]
    # total_grade = sum(grades_int)
    # target_grade_per_class = total_grade // k
    # 
    # # ±3% 허용 범위 (더 엄격한 균형)
    # tolerance_pct = 0.03
    # tolerance = int(target_grade_per_class * tolerance_pct)
    # min_grade_per_class = target_grade_per_class - tolerance
    # max_grade_per_class = target_grade_per_class + tolerance
    # 
    # print(f"Grade balancing (RELAXED ±3%): total={total_grade}, target per class={target_grade_per_class} ±{tolerance}")
    # 
    # # 각 학급의 성적 합을 소프트 제약으로
    # grade_violation_vars = []
    # for c in range(k):
    #     grade_sum = sum(x[(i,c)] * grades_int[i] for i in range(n))
    #     
    #     # 하한 위반
    #     under_var = model.NewIntVar(0, total_grade, f'grade_under_{c}')
    #     model.Add(under_var >= min_grade_per_class - grade_sum)
    #     model.Add(under_var >= 0)
    #     grade_violation_vars.append(under_var)
    #     
    #     # 상한 위반
    #     over_var = model.NewIntVar(0, total_grade, f'grade_over_{c}')
    #     model.Add(over_var >= grade_sum - max_grade_per_class)
    #     model.Add(over_var >= 0)
    #     grade_violation_vars.append(over_var)

    # print("Added relaxed grade balance constraints (SOFT)")

    # 목적함수: 성적 제약 없이 단순화 (전년도 분리는 하드 제약으로 처리됨)
    # total_grade_violations = sum(grade_violation_vars)
    # 
    # model.Minimize(total_grade_violations)
    print("No grade balance objective (previous classmate separation handled as HARD constraint)")
    ...
    if status in (cp_model.OPTIMAL, cp_model.FEASIBLE):
        ...
        for c in range(k):
            ...
            print(f"학급 {c}: {len(members)}명, 리더 {leaders}명, 피아노 {pianos}명, 비등교 {atrisks}명, 남 {males}명, 여 {females}명, 운동 {athletics}명, 클럽 {clubs}명")
            print(f"        평균성적 {grade_avg:.1f}, 총점 {grade_sum:.0f}")
        
        # 전년도 클래스메이트 분리 효과 (하드 제약으로 100% 분리 보장)
        if previous_pairs:
            print(f"\n전년도 클래스메이트 분리: 0/{len(previous_pairs)} 위반 (분리율: 100.0%) - HARD constraint로 완전 분리")
        
        # 성적 균형 분석 (제약 없음)
        grade_std = np.std(grade_sums)
        grade_range = max(grade_sums) - min(grade_sums)
        grade_avg_total = sum(grade_sums) / len(grade_sums)
        
        print(f"\n성적 균형 분석 (제약 없음):")
        print(f"  표준편차: {grade_std:.1f}")
        print(f"  최대-최소 차이: {grade_range:.0f}")  
        print(f"  평균 총점: {grade_avg_total:.0f}")
...
def main():
    parser = argparse.ArgumentParser(description="Class assignment - Relaxed grade balance version")
    ...
    # OR-Tools 실행
    success = run_ortools_relaxed(df, args.output, class_sizes)
    if not success:
        print("\n💡 실패 원인을 분석해보세요.")
