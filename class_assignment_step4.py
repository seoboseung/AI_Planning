#!/usr/bin/env python3
"""
class_assignment_step4.py - 리더십 분배 제약조건 추가
...
Usage:
    python class_assignment_step4.py --input students.csv --output assignment.csv --mode ortools
"""
...
def run_ortools_step4(df, out_path, class_sizes):
    try:
    ...
    if status in (cp_model.OPTIMAL, cp_model.FEASIBLE):
        ...
        for c in range(k):
            ...
            print(f"학급 {c}: {len(members)}명, 리더 {leaders}명")
...
def main():
    parser = argparse.ArgumentParser(description="Class assignment - Step 4 (Leadership)")
    ...
    # 리더십 제약조건 실행 가능성 체크
    if total_leaders < len(class_sizes):
        print(f"⚠️ 경고: 리더 수({total_leaders})가 학급 수({len(class_sizes)})보다 적습니다!")
        print("   각 학급에 최소 1명씩 배정하는 것이 불가능할 수 있습니다.")

    # OR-Tools 실행
    success = run_ortools_step4(df, args.output, class_sizes)
    if not success:
        print("\n💡 실패 원인을 분석해보세요.")
