#!/usr/bin/env python3
"""
class_assignment_step7.py - 비등교자 균등 분배 추가

Step 7: 비등교자 균등 분배 추가
...
Usage:
    python class_assignment_step7.py --input students.csv --output assignment.csv --mode ortools
"""
...
def run_ortools_step7(df, out_path, class_sizes):
    try:
    ...
    if status in (cp_model.OPTIMAL, cp_model.FEASIBLE):
        ...
        for c in range(k):
            ...
            print(f"학급 {c}: {len(members)}명, 리더 {leaders}명, 피아노 {pianos}명, 비등교 {atrisks}명, 평균성적 {grade_avg:.1f}, 총점 {grade_sum:.0f}")
...
def main():
    parser = argparse.ArgumentParser(description="Class assignment - Step 7 (At-risk balance)")
    ...
    # OR-Tools 실행
    success = run_ortools_step7(df, args.output, class_sizes)
    if not success:
        print("\n💡 실패 원인을 분석해보세요.")
