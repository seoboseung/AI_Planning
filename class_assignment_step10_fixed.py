#!/usr/bin/env python3
"""
class_assignment_step10_fixed.py - 전년도 클래스메이트 분리 수정

Step 10: 전년도 클래스메이트 분리 추가 (24년 학급 데이터 활용)
...
Usage:
    python class_assignment_step10_fixed.py --input students.csv --output assignment.csv --mode ortools
"""
...
def run_ortools_step10_fixed(df, out_path, class_sizes):
    try:
...
def main():
    parser = argparse.ArgumentParser(description="Class assignment - Step 10 Fixed (Previous classmate separation using 24년 학급)")
    ...
    print("=== Step 10 Fixed: 전년도 클래스메이트 분리 (24년 학급 활용) ===")
    print(f"감지된 컬럼들:")
    print(f"  ID: {id_col}")
    ...
    # OR-Tools 실행
    success = run_ortools_step10_fixed(df, args.output, class_sizes)
    if not success:
        print("\n💡 실패 원인을 분석해보세요.")
